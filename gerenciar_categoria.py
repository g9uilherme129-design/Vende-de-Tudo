...
def gerenciar_categorias(page: ft.Page, on_back):
    ...
    def criar_card_categoria(c):
        ...
        return ft.Container(
            ...
            padding=15,
            content=ft.Row([
                ft.Icon(ft.Icons.CATEGORY_ROUNDED, color=ft.Colors.TEAL_400),
                ft.Column([
                    ft.Text(c["nome_categoria"].upper(), size=16, weight="bold", color=cor_texto_principal),
                    ft.Text(f"ID: {c['id_categoria']}", size=11, color=cor_texto_secundario),
                ], expand=True),
                ft.Container(
    ...
    def filtrar_categorias(e=None):
        ...
        filtrados = [
            c for c in categorias_base 
            if texto in c["nome_categoria"].lower()
        ]
    ...
    def abrir_modal_novo(e):
        nome_input = ft.TextField(label="Nome da Categoria", border_radius=10)
        ...
        dlg = ft.AlertDialog(
            ...
            content=nome_input,
            actions=[ft.TextButton("Cancelar", on_click=lambda _: setattr(dlg, "open", False)),
    ...
    def abrir_modal_edicao(cat):
        nome_edit = ft.TextField(label="Nome", value=cat["nome_categoria"], border_radius=10)
        ...
        dlg = ft.AlertDialog(
            ...
            content=nome_edit,
            actions=[ft.ElevatedButton("Atualizar", on_click=atualizar)]
        )
        page.overlay.append(dlg)
        dlg.open = True
        page.update()
    ...
    page.add(
        ft.Column(expand=True, controls=[
            ft.Row([
                ft.Text("Gestão de Categorias", size=24, weight="bold"),
                ft.FloatingActionButton(icon=ft.Icons.ADD, bgcolor="#1B4F9C", mini=True, on_click=abrir_modal_novo)
