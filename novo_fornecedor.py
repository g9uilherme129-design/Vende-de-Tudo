...
def novo_fornecedor(page: ft.Page, on_voltar):
    ...
    def estilo_input(label, hint="", value="", col=None, keyboard_type=ft.KeyboardType.TEXT, limite=None, is_dropdown=False):
        ...
        container = ft.Column([
            ...
            ft.Container(
                ...
                height=50 if is_dropdown else None # Mantém o alinhamento visual
            )
        ], spacing=5, col=col)
    ...
    bairro_c, bairro_in = estilo_input("BAIRRO", col=12, limite=100)
    cidade_c, cidade_in = estilo_input("CIDADE", col=8, limite=100)
    
    # ESTADO como Dropdown
    uf_c, uf_in = estilo_input("ESTADO (UF)", value="--", col=4, is_dropdown=True)
    
    cep_c, cep_in = estilo_input("CEP", hint="00000-000", col=4, limite=9)
    ...
    corpo_formulario = ft.Container(
        ...
        width=550, # Aumentei um pouco para acomodar melhor os campos lado a lado
        padding=10,
    )
