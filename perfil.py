...
def perfil_page(page: ft.Page, on_home, on_stock, on_users, on_logout, on_config):
    ...
    # 1. Recupera dados da sessão (definidos no login_view)
    # Usamos .get() com um valor padrão caso a sessão esteja vazia
    nome_usuario = page.session.get("user_name") or "Usuário"
    perfil_usuario = page.session.get("user_perfil") or "Visitante"
    id_usuario = page.session.get("user_id") or "000"
    ...
    # --- AVATAR E INFO PRINCIPAL (AGORA DINÂMICO) ---
    avatar = ft.Container(
    ...
    user_info = ft.Column(
        [
            ...
            ft.Text(perfil_usuario.upper(), size=14, color="#00b0ff"),
            ft.Container(
                content=ft.Text(f"ID: VT-{id_usuario}", size=12, color="white"),
                bgcolor="#1e3a8a", padding=ft.padding.symmetric(horizontal=12, vertical=4),
                border_radius=20,
            ),
        ],
    ...
    def criar_item_menu(icon, label, color="#2196f3", on_click=None):
        return ft.Container(
            padding=15, border_radius=15, bgcolor=cor_fundo_item,
    ...
    layout_acoes = ft.ResponsiveRow(
        controls=[
            criar_item_menu(ft.Icons.CLOUD_OUTLINED, "Sincronização Cloud", on_click=lambda _: print("Sync")),
            criar_item_menu(ft.Icons.SETTINGS_OUTLINED, "Configurações do App", on_click=lambda _: on_config()),
    ...
    page.add(ft.Container(content=conteudo, padding=25, expand=True))

    # --- NAVIGATION BAR ---
    nav = ft.NavigationBar(
        bgcolor="#0b1445",
        selected_index=3,
        on_change=lambda e: [on_home(), on_stock(), on_users(), None][e.control.selected_index],
        destinations=[
